#' Supremum1 distance between multivariate functions
#'
#' Computes the Supremum1 distance for all pairs of \eqn{m}-dimensional functions.
#' The Supremum1 distance returns the sum of the supremum distances between the function values at equal time points.
#'
...
#' @details For each pair of functions f and g, the present R function computes: \eqn{sum t [Supremum_Distance(f(t), g(t))]}
...
#' data = list(x, y)
#' msupremum1(data, parallel = FALSE, cl = NULL)
#'
...
msupremum1=function(data,parallel=FALSE,cl=NULL)
{
  ...
  if(parallel==FALSE)
  {
    return(apply(cbind(1:n),1,function(i) colSums(apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-", list(as.matrix(data[[j]]),as.matrix(data[[j]])[,i]))))),c(1,2),max))))
  } else {
    cl=parallel::makeCluster(cl)
    parallel::clusterExport(cl,list("data"))
    return(parallel::parApply(cl,cbind(1:n),1,function(i) colSums(apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-", list(as.matrix(data[[j]]),as.matrix(data[[j]])[,i]))))),c(1,2),max))))
