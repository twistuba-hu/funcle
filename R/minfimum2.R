#' Infimum2 distance between multivariate functions
#'
#' Computes the Infimum2 distance for all pairs of \eqn{m}-dimensional functions. For a single pair of functions,
#' the Infimum2 distance returns the minimum value of the infimum distances between the function values at equal time points.
#'
...
#' @return Returns a square and symmetric \eqn{n x n} matrix of \eqn{m}-dimensional Infimum2 distances.
#' @details For each pair of functions f and g, the present R function computes: \eqn{max t [Infimum_Distance(f(t), g(t))]}
...
#' data = list(x, y)
#' minfimum2(data, parallel = FALSE, cl = NULL)
#'
...
minfimum2=function(data,parallel=FALSE,cl=NULL)
{
  ...
  if(parallel==FALSE)
  {
    return(apply(cbind(1:n),1,function(i)apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-",list(data[[j]],data[[j]][,i]))))),2,min)))
  } else {
    cl=parallel::makeCluster(cl)
    parallel::clusterExport(cl,list("data"))
    return(parallel::parApply(cl=cl,cbind(1:n),1,function(i)apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-",list(data[[j]],data[[j]][,i]))))),2,min)))
