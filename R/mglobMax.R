...
mglobMax=function(data,parallel=FALSE,cl=NULL)
{
  ...
  if(parallel==FALSE){
    return(matrix(apply(cbind(1:n),1,function(i) sqrt(colSums((t(sapply(data,function(x) Rfast::colMaxs(as.matrix(x),value=TRUE)))-t(sapply(data,function(x) Rfast::colMaxs(as.matrix(x),value=TRUE)))[,i])^2))),ncol=n,nrow=n,byrow=TRUE))
  } else {
