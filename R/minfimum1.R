#' Infimum1 distance between multivariate functions
#'
#' Computes the Infimum1 distance for all pairs of \eqn{m}-dimensional functions. For a single pair of functions,
#' the Infimum1 distance returns the sum of the infimum distances between the function values at equal time points.
#'
...
#' @return Returns a square and symmetric \eqn{n x n} matrix of \eqn{m}-dimensional Infimum1 distances.
#' @details For each pair of functions f and g, the present R function computes: \eqn{sum t [Infimum_Distance(f(t), g(t))]}
...
#' data = list(x, y)
#' minfimum1(data, parallel = FALSE, cl = NULL)
#'
...
#'
#'
minfimum1=function(data,parallel=FALSE,cl=NULL)
{
  ...
  if(parallel==FALSE)
  {
    return(apply(cbind(1:n),1,function(i) colSums(apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-",list(data[[j]],data[[j]][,i]))))),c(1,2),min))))
  } else {
    cl=parallel::makeCluster(cl)
    parallel::clusterExport(cl,list("data"))
    return(parallel::parApply(cl=cl,cbind(1:n),1,function(i) colSums(apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-",list(data[[j]],data[[j]][,i]))))),c(1,2),min))))
    parallel::stopCluster(cl)}
}

# x = replicate(4, rnorm(4, 0, 3))
# y = replicate(4, rnorm(4, 3, 1))
# data = list(x, y)
# minfimum1(data, parallel = FALSE, cl = NULL)
# i=1
# plyr::alply(cbind(1:d),1,function(j) abs(do.call("-",list(data[[j]],data[[j]][,i]))))
# apply(simplify2array(plyr::alply(cbind(1:d),1,function(j) abs(do.call("-",list(data[[j]],data[[j]][,i]))))),c(1,2),min)
